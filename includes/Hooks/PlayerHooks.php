	...
	SkinTemplateNavigation__UniversalHook
{
	...
	public function onBeforePageDisplay( $out, $skin ): void {
		...
		$out->addJsConfigVars( [
			...
			'wgWikispeechUseBrowserTTS' => $this->config->get( 'WikispeechUseBrowserTTS' )
		] );
	}
	 ...
	 * * Wikispeech configuration is valid (or browser TTS is enabled)
	 * * Wikispeech is enabled for the page's namespace
	 * * Revision is current
	 * * Page's language is enabled for Wikispeech (or browser TTS is enabled)
	 * * The action is "view"
	 *
	 * @since 0.1.11
	...
	private function shouldWikispeechRun( OutputPage $out ) {
		...
		$useBrowserTTS = $this->config->get( 'WikispeechUseBrowserTTS' );

		if ( !$useBrowserTTS && !$this->configValidator->validateConfiguration() ) {
		...
		// Skip language validation for browser TTS as it supports more languages
		if ( !$useBrowserTTS ) {
	...
	public function onResourceLoaderGetConfigVars( array &$vars, $skin, Config $config ): void {
		...
		$vars['wgWikispeechUseBrowserTTS'] = $config->get( 'WikispeechUseBrowserTTS' );
	}
	...
	public function onGetPreferences( $user, &$preferences ) {
		// Only create Speechoid connector if not using browser TTS
		$useBrowserTTS = $this->config->get( 'WikispeechUseBrowserTTS' );
		$voiceHandler = null;
		...
		// Add browser TTS preference if enabled in config
		if ( $useBrowserTTS ) {
			$preferences['wikispeechUseBrowserTTS'] = [
				'type' => 'toggle',
				'label-message' => 'prefs-wikispeech-use-browser-tts',
				'section' => 'wikispeech',
				'default' => true,
				'disabled' => true // Read-only, controlled by config
			];
		}
	...
	public function onSkinTemplateNavigation__Universal( $skinTemplate, &$links ): void { // phpcs:ignore MediaWiki.NamingConventions.LowerCamelFunctionsName.FunctionName, Generic.Files.LineLength.TooLong
		...
		if ( $this->shouldWikispeechRun( $out ) ) {
			// 在浏览器TTS模式下，监听标签总是显示，因为需要加载模块来检查支持状态
