...
function Storage() {
	...
	/**
	 * Request TTS for a hash.
	 *
	 * @param {string} hash The hash for the utterance.
	 * @param {Object} window
	 */

	this.requestTts = function ( hash, window ) {
		// Check if using browser TTS
		if ( mw.config.get( 'wgWikispeechUseBrowserTTS' ) ) {
			// For browser TTS, we still need to get segments but don't need audio
			return self.requestTtsForBrowserTTS( hash, window );
		}
	...
	/**
	 * Request TTS data for browser TTS mode
	 */
	this.requestTtsForBrowserTTS = function ( hash, window ) {
		const utterance = self.getUtteranceByHash( hash );
		if ( !utterance ) {
			console.error( '[Wikispeech] Utterance not found for hash:', hash );
			return $.Deferred().reject( 'Utterance not found' );
		}

		// Create fake response for browser TTS
		const response = {
			'wikispeech-listen': {
				audio: 'BROWSER_TTS_PLACEHOLDER',
				tokens: self.generateTokensFromUtterance( utterance )
			}
		};

		// Create a resolved deferred that mimics the API response
		const deferred = $.Deferred();
		
		// Use setTimeout to make it asynchronous
		setTimeout( () => {
			mw.log( '[Wikispeech] Browser TTS response generated:', response );
	...
	/**
	 * Generate tokens from utterance for browser TTS compatibility
	 */
	this.generateTokensFromUtterance = function ( utterance ) {
		const tokens = [];
		let currentTime = 0;
		
		if ( utterance && utterance.content ) {
			utterance.content.forEach( item => {
				const words = item.string.split( /\s+/ );
				words.forEach( word => {
					if ( word.trim() ) {
						const duration = word.length * 100; // 100ms per character
						tokens.push( {
							orth: word,
							starttime: currentTime,
							endtime: currentTime + duration
						} );
						currentTime += duration + 100; // 100ms pause between words
					}
				} );
			} );
		}
	...
	/**
	 * Get utterance by hash
	 */
	this.getUtteranceByHash = function ( hash ) {
		return self.utterances.find( utterance => utterance.hash === hash );
	};
	...
	this.getNextToken = function ( originalToken ) {
		if ( !originalToken || !originalToken.utterance || !originalToken.utterance.tokens ) {
			console.warn( '[Wikispeech] getNextToken called with invalid token:', originalToken );
			return null;
		}

		const index = originalToken.utterance.tokens.indexOf( originalToken );
		if ( index === -1 ) {
			console.warn( '[Wikispeech] Token not found in utterance tokens' );
			return null;
		}
	...
	this.getPreviousToken = function ( originalToken ) {
		if ( !originalToken || !originalToken.utterance || !originalToken.utterance.tokens ) {
			console.warn( '[Wikispeech] getPreviousToken called with invalid token:', originalToken );
			return null;
		}
	...
	this.getLastToken = function ( utterance ) {
		if ( !utterance || !utterance.tokens || utterance.tokens.length === 0 ) {
			console.warn( '[Wikispeech] getLastToken called with invalid utterance:', utterance );
			return null;
		}

		const nonSilentTokens = utterance.tokens.filter( ( token ) => !self.isSilent( token ) );
		if ( nonSilentTokens.length === 0 ) {
			return null;
		}
