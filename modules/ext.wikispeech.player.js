...
function Player() {
	...
	this.pauseUtterance = function ( utterance ) {
		// Check if using browser TTS
		if ( mw.config.get( 'wgWikispeechUseBrowserTTS' ) ) {
			mw.wikispeech.browserTTS.pause();
			mw.wikispeech.ui.setAllPlayerIconsToPlay();
			return;
		}
	...
	/**
	 * Resume the current utterance.
	 */

	this.resumeUtterance = function () {
		// Check if using browser TTS
		if ( mw.config.get( 'wgWikispeechUseBrowserTTS' ) ) {
			mw.wikispeech.browserTTS.resume();
			mw.wikispeech.ui.setPlayPauseIconToPause();
			return;
		}

		// Resume logic for non-browser TTS
		if ( self.paused ) {
	...
	this.playUtterance = function ( utterance, fromStart ) {
		...
		// Check if using browser TTS
		if ( mw.config.get( 'wgWikispeechUseBrowserTTS' ) && utterance ) {
			self.playWithBrowserTTS( utterance );
			return;
		}
	...
	/**
	 * Play utterance using browser TTS
	 */
	this.playWithBrowserTTS = function ( utterance ) {
		if ( !utterance ) {
			console.warn( '[Wikispeech] No utterance to play' );
			return;
		}

		// Extract text from utterance content
		let text = '';
		if ( utterance.content && utterance.content.length > 0 ) {
			text = utterance.content.map( item => item.string ).join( ' ' );
		}
		
		if ( !text.trim() ) {
			console.warn( '[Wikispeech] No text to speak' );
			// Try to get text from tokens if available
			if ( utterance.tokens && utterance.tokens.length > 0 ) {
				text = utterance.tokens.map( token => token.string || token.orth ).join( ' ' );
			}
			
			if ( !text.trim() ) {
				console.error( '[Wikispeech] Still no text found to speak' );
				self.stop();
				return;
			}
		}

		const language = mw.config.get( 'wgPageContentLanguage' ) || 'en';

		console.log( '[Wikispeech] Playing with browser TTS:', {
		...
		mw.wikispeech.browserTTS.speak( text, language )
			.then( () => {
				// Speech ended naturally
				console.log( '[Wikispeech] Browser TTS completed for utterance' );
				
				// Only proceed to next utterance if still playing the same one
				if ( self.currentUtterance === utterance ) {
					const nextUtterance = mw.wikispeech.storage.getNextUtterance( utterance );
					if ( nextUtterance ) {
			...
			.catch( ( error ) => {
				// Only log error if it's not an intentional stop
				if ( error && error.error !== 'interrupted' ) {
					console.error( '[Wikispeech] Browser TTS playback failed:', error );
				}
				// Don't automatically stop here if it was intentional
				if ( self.currentUtterance === utterance && error && error.error !== 'interrupted' ) {
					self.stop();
				}
			} );
	};
	...
	this.skipBackUtterance = function () {
		// For browser TTS, we need to handle this differently
		if ( mw.config.get( 'wgWikispeechUseBrowserTTS' ) ) {
			const previousUtterance = mw.wikispeech.storage.getPreviousUtterance( self.currentUtterance );
			if ( previousUtterance ) {
				self.playUtterance( previousUtterance );
			} else {
				// If it's the first utterance, restart it
				self.stop();
				self.playUtterance( self.currentUtterance );
			}
			return;
		}
	...
	this.skipAheadToken = function () {
		...
		// For browser TTS, skip to next utterance since token-level control isn't available
		if ( mw.config.get( 'wgWikispeechUseBrowserTTS' ) ) {
		...
		const currentToken = self.getCurrentToken();
		if ( !currentToken ) {
			// If no current token, skip to next utterance
			self.skipAheadUtterance();
			return;
		}
	...
	this.skipBackToken = function () {
		...
		// For browser TTS, skip to previous utterance since token-level control isn't available
		if ( mw.config.get( 'wgWikispeechUseBrowserTTS' ) ) {
		...
		const currentToken = self.getCurrentToken();
		if ( !currentToken ) {
			// If no current token, skip to previous utterance
			self.skipBackUtterance();
			return;
		}
